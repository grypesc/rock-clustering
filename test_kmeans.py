...
if __name__ == '__main__':
    ...
    agg = KMeans(n_clusters=2).fit(data[:, :])
    ...
    agg = KMeans(n_clusters=2).fit(categorical_to_binary(data))
    ...
    fig.add_child(map)
    colors = ['green', 'gray', 'orange', 'darkred', 'blue', 'black',  'lightblue', 'lightred',  'darkpurple', 'darkgreen',
              'beige',
              'red', 'lightgreen', 'cadetblue', 'lightgray', 'darkblue', 'pink']

    clustering = KMeans(n_clusters=6, n_init=50, random_state=123).fit(data)
    ...
    map.save('docs/nyc_clustering_kmeans.html')
    ...
    clustering = KMeans(n_clusters=3, n_init=50, random_state=123).fit(data)
